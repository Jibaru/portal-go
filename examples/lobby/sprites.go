...
// (3 walk frames of 16x18 per direction row; rows: up, right, down, left).
...
// each 48x72 — 3 frames of 16x18 per direction row, rows up/right/down/left.
func loadCharacterSheet(data []byte) error {
	...
	// Our dirs (0 up, 1 right, 2 down, 3 left) → sheet rows. The sheet's rows
	// top→bottom are up, right, down, left — a direct 1:1 mapping.
	rowForDir := [4]int{0, 1, 2, 3}
